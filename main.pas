...
begin
    ...
    repeat
        ...
        repeat
            write('Limit a: ');
            readln(userValue);
            if length(userValue) > 5 then begin
        ...
        repeat
            ...
            val(userValue, b, i);
            if i <> 0 then 
                errorMessage('Check if there are any extraneous characters in the input or replace "," to "."');
        until i = 0;
        if a > b then begin
            a:=a+b; b:=a-b; a:=a-b;
        end;
    ...
    until i = 0;
    if a < xRoot then a:= xRoot;
    if b < xRoot then b:= xRoot;
    textColor(Green);
...
function primitivef(xr:currency):currency; { первообразная функции}
begin
...
begin
    ...
    repeat
        putImage(20,20,pFrame^,1);
    xReal:=(xn-20)/10;
    for i:=-5 to 4 do begin
        x:=trunc(i*xReal);
...
procedure trapArea;   { Непосредственно вычисление самого алгоитма и вывод}
var x:real;
begin
    newPage(menu[selectedItem]);
    x:=a+h;
    area:=(f(a)+f(b))/2;
    while (b-x) > t do begin
        area:=area+f(x);
        x:=x+h;
    end;
    area:=area * h;
            ...
            4: begin
                if complite then
                    ...
                    if status then
                        ...
                        errorMessage('You must input limits, number of intervals and then exec "Area"');
            end;
            5: about;
            6: quit;
        end;
